...
SoapySDR::Stream::~Stream()
{
	...
	filter_q = NULL;
        ...
        stream_active = 0;
        reader_thread_running = 0;
        if (this->radio->channel_direction == cariboulite_channel_dir_rx)
        {
            reader_thread->join();
...
{
	int ret = cariboulite_radio_write_samples(radio, buffer, num_samples);

    if (ret < 0)
    {
        if (ret == -1)
        {
            printf("SoapySDR::Stream::Write failed, return: %d\n", ret);
        }
        ret = 0;
    }
    return ret;
...
int SoapySDR::Stream::Read(caribou_smi_sample_complex_int16 *buffer, size_t num_samples, uint8_t *meta, long timeout_us)
{
        ...
        if (ret < 0)
        {
            if (ret == -1)
            {
                printf("SoapySDR::Stream::Read: reader thread failed to read SMI!\n");
            }
            // a special case for debug streams which are not
            // taken care of in the soapy front-end (ret = -2)
            ret = 0;
...
{
    int res = Write(buffer, num_elements, NULL, timeout_us);
    if (res < 0)
    {
        //SoapySDR_logf(SOAPY_SDR_ERROR, "Reading %d elements failed from queue", num_elements);
        return res;
...
int SoapySDR::Stream::WriteSamples(sample_complex_float* buffer, size_t num_elements, long timeout_us)
{
    ...
    {
        // Assuming normalized float64 samples (i.e. max()=1)
        interm_native_buffer2[i].i = (int16_t)(round(buffer[i].i * max_val));
        interm_native_buffer2[i].q = (int16_t)(round(buffer[i].q * max_val));
    }

    int res = WriteSamples(interm_native_buffer2, num_elements, timeout_us);
...
int SoapySDR::Stream::WriteSamples(sample_complex_int8* buffer, size_t num_elements, long timeout_us)
{
    ...
    for (size_t i = 0; i < num_elements; i++)
    {
        interm_native_buffer2[i].i = (int16_t)((buffer[i].i << 5)&0x01FE0);
        interm_native_buffer2[i].q = (int16_t)((buffer[i].q << 5)&0x01FE0);
    }
...
int SoapySDR::Stream::WriteSamplesGen(void* buffer, size_t num_elements, long timeout_us)
{
    ...
    SoapySDR_logf(SOAPY_SDR_INFO, "SoapySDR::Stream::WriteSamplesGen: writing %d elements", num_elements);
    switch (format)
    {
